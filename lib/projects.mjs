...
async function detectProjectsInRoot(rootInfo, logger) {
  const out = []
  ...
  async function walk(currentDir, relDir) {
    ...
    for (const ent of entries) {
      ...
      if (!relDir && ent.name === 'issue_manager') {
        const selfTickets = path.join(projPath, '_self', 'tickets')
        if (fs.existsSync(selfTickets)) {
          const info = await inspectTicketsDir(path.join(projPath, '_self'), `${ent.name}/_self`)
          if (info) out.push(withRootInfo(info, rootInfo))
        }
        continue
      }
      ...
      if (fs.existsSync(newTickets)) {
        ...
        const info = await inspectTicketsDir(projPath, childRel)
        if (info) { out.push(withRootInfo(info, rootInfo)); continue }
      }
...
export async function detectProjects(rootsOrRoot, logger) {
  const roots = Array.isArray(rootsOrRoot)
    ? rootsOrRoot
    : normalizeRoots({ root: rootsOrRoot })
  const out = []
  for (const rootInfo of roots) {
    out.push(...await detectProjectsInRoot(rootInfo, logger))
  }
  return out
}
...
export async function getProjectById(id, roots, logger) {
  const projects = await detectProjects(roots, logger)
