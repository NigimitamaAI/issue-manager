...
> [!IMPORTANT]
> - **外部アセットと拡張機能の扱い**: 
>   `enterprise/`（拡張機能）や `_private/`（非公開仕様等）、チケットデータは、EXE内部にパックするとユーザーが編集できなくなるため、**「EXEファイルと同じディレクトリに置かれた外部ファイル」**としてロードする設計にします。
> - **設定ファイル `config.json`**:
...
3. テストフォルダに、ダミーの `config.json` と `enterprise/preview-lane` ディレクトリをコピーします。
...
5. ブラウザから管理画面にアクセスし、「プレビューレーン」ボタンが表示され、問題なく動作することを確認します。
