...
const MOVE_HISTORY_FILE = 'move-history.json'
const MOVE_HISTORY_SCHEMA = 'issue-manager-project-move-history-v1'
...
export function makeHandler(ctx) {
  ...
  async function appendProjectMoveHistory(issuemgrDir, entry) {
    const historyPath = path.join(issuemgrDir, MOVE_HISTORY_FILE)
    let history = { schema: MOVE_HISTORY_SCHEMA, entries: [] }
    if (fs.existsSync(historyPath)) {
      try {
        const parsed = JSON.parse((await fsp.readFile(historyPath, 'utf8')).replace(/^\uFEFF/, ''))
        if (parsed && typeof parsed === 'object' && Array.isArray(parsed.entries)) {
          history = {
            schema: parsed.schema || MOVE_HISTORY_SCHEMA,
            entries: parsed.entries,
          }
        }
      } catch (e) {
        history.readError = String(e && e.message || e)
      }
    }
    history.schema = MOVE_HISTORY_SCHEMA
  ...
  function readMoveHistorySummary(project) {
    const full = path.join(project.projectDir, '.issuemgr', MOVE_HISTORY_FILE)
    ...
    try {
      ...
      return {
        ...
        latest: latest ? {
          type: latest.type || '',
          recordedAt: latest.recordedAt || latest.movedAt || '',
          from: latest.from || null,
          to: latest.to || null,
          moveContext: latest.moveContext || null,
        } : null,
      }
    } catch (e) {
      return {
  ...
  function projectJson(p) {
    ...
    return {
      ...
      moveHistory: readMoveHistorySummary(p),
      commonRules: commonRulesJson(),
    }
  }

  const PATH_AUDIT_SKIP_DIRS = new Set([
    '.git',
    '.svn',
    '.hg',
    'node_modules',
  ...
  function shouldAuditPath(relPath) {
    const rel = relPath.replace(/\\/g, '/')
    const base = path.basename(rel)
    return rel.startsWith('tickets/')
      || rel.startsWith('.issuemgr/') && base.endsWith('.json')
      || rel === '.well-known/issue-manager.json'
  ...
  function extractPathReferences(text) {
    const patterns = [
      { kind: 'windows', re: /\b[A-Za-z]:[\\/][^\s"'`<>)\]}]+/g },
      { kind: 'wsl', re: /\/mnt\/[A-Za-z]\/[^\s"'`<>)\]}]+/g },
      { kind: 'file-uri', re: /file:\/\/\/[A-Za-z]:\/[^\s"'`<>)\]}]+/g },
    ]
    const refs = []
    for (const pattern of patterns) {
      for (const match of text.matchAll(pattern.re)) {
        ...
        const snippet = text.slice(lineStart, lineEnd).replace(/\r$/, '').trim()
        refs.push({
          kind: pattern.kind,
          value: match[0],
          index,
  ...
  function pathAuditMarkdown(result) {
    ...
    const lines = [
      ...
      '- projectName: ' + (project.projectName || project.displayName || project.name || ''),
      '- projectDir: ' + (project.projectDir || ''),
      '- matchFiles: ' + ((result.matches || []).length),
    ...
    for (const item of result.matches || []) {
      ...
      lines.push('')
      if (item.truncated) {
        lines.push('> このファイルは候補が多いため一部のみ記録しています。')
        lines.push('')
      }
      for (const ref of item.refs || []) {
        ...
        if (ref.snippet) lines.push('  - snippet: `' + ref.snippet.replace(/`/g, '\\`') + '`')
      }
      lines.push('')
    }
    if (!result.matches || !result.matches.length) {
      lines.push('絶対パス候補は見つかりませんでした。')
      lines.push('')
    }
    return lines.join('\n')
  }

  async function savePathAuditReport(project, result) {
    const reportDir = path.join(project.projectDir, '.issuemgr', 'path-audit')
    await fsp.mkdir(reportDir, { recursive: true })
    const stamp = nowStamp()
    const base = `path-audit_${stamp}`
    ...
    await fsp.writeFile(mdPath, pathAuditMarkdown(payload), 'utf8')
    return {
      reportDir,
      jsonPath,
      mdPath,
      jsonRel: path.relative(project.projectDir, jsonPath).replace(/\\/g, '/'),
      mdRel: path.relative(project.projectDir, mdPath).replace(/\\/g, '/'),
    }
  }

  async function scanProjectPathReferences(project) {
    const matches = []
    async function walk(dir) {
      ...
      for (const ent of entries) {
        if (ent.isDirectory() && PATH_AUDIT_SKIP_DIRS.has(ent.name)) continue
        const full = path.join(dir, ent.name)
        const rel = path.relative(project.projectDir, full).replace(/\\/g, '/')
        if (ent.isDirectory()) {
          await walk(full)
          continue
        }
        if (!ent.isFile() || !shouldAuditPath(rel)) continue
        let stat
        try {
        ...
        if (refs.length) {
          matches.push({
            ...
            truncated: refs.length > 50,
          })
        }
      }
    }
    await walk(project.projectDir)
    return {
    ...
    try {
      ...
      if (pathname === '/api/fs/children' && method === 'GET') {
        ...
        for (const ent of await fsp.readdir(current.full, { withFileTypes: true })) {
          ...
          const unregisteredPath = path.join(full, '.issuemgr', UNREGISTERED_FILE)
          entries.push({
            name: ent.name,
            path: relPath,
            managed: managedDirs.has(path.resolve(full).toLowerCase()),
            hasTickets: fs.existsSync(path.join(full, 'tickets')),
            unregistered: fs.existsSync(unregisteredPath),
          })
        }
      ...
      if (pathname === '/api/projects/import' && method === 'POST') {
        ...
        if (fs.existsSync(path.join(projDir, 'tickets'))) {
          const issuemgrDir = path.join(projDir, '.issuemgr')
          const unregisteredPath = path.join(issuemgrDir, UNREGISTERED_FILE)
          if (!fs.existsSync(unregisteredPath)) {
            return sendJson(res, 409, { error: '既に tickets ディレクトリが存在します' })
          }
          const restoredDir = path.join(issuemgrDir, 'unregistered')
          await fsp.mkdir(restoredDir, { recursive: true })
          const restoredPath = path.join(restoredDir, `restored_${nowStamp()}.json`)
          await fsp.rename(unregisteredPath, restoredPath)
          const restoredAt = new Date().toISOString()
          const id = `${rootInfo.id}~${encodeURIComponent(target.rel)}`
          await appendProjectMoveHistory(issuemgrDir, {
            type: 'reregister',
            ...
            to: {
              ...
              rootPath: rootInfo.path,
              projectDir: projDir,
              ticketsDir: path.join(projDir, 'tickets'),
            },
            restoredUnregisteredPath: path.relative(projDir, restoredPath).replace(/\\/g, '/'),
          })
          await SMD.generateAllStatusMd()
          return sendJson(res, 200, {
            ok: true,
      ...
      if (/^\/api\/projects\/[^/]+\/unregister$/.test(pathname) && method === 'POST') {
        ...
        const rootInfo = rootForProject(project) || {}
        const unregisteredFrom = {
          projectId: project.id,
          projectName: project.projectName || project.name,
          rootId: project.rootId || '',
          rootLabel: project.rootLabel || '',
          rootPath: project.rootPath || (rootInfo && rootInfo.path) || '',
          projectDir: project.projectDir,
          ticketsDir: project.ticketsDir,
        }
        const moveContext = {
          reason: typeof body.reason === 'string' ? body.reason.trim() : '',
          intendedMoveTo: body.intendedMoveTo && typeof body.intendedMoveTo === 'object'
            ? body.intendedMoveTo
            : null,
          nextPathHint: typeof body.nextPathHint === 'string' ? body.nextPathHint.trim() : '',
        }
        const payload = {
          unregisteredAt,
          mode,
        ...
        await appendProjectMoveHistory(issuemgrDir, {
          type: 'unregister',
          recordedAt: unregisteredAt,
          mode,
          from: unregisteredFrom,
          moveContext,
          archivePath: archivePath ? path.relative(project.projectDir, archivePath).replace(/\\/g, '/') : '',
        })
        await SMD.generateAllStatusMd()
      ...
      let m = pathname.match(/^\/api\/projects\/([^/]+)\/path-audit$/)
      if (m && method === 'GET') {
        const project = await getProject(decodeURIComponent(m[1]))
        if (!project) return sendJson(res, 404, { error: 'project not found' })
        return sendJson(res, 200, await scanProjectPathReferences(project))
      }

      m = pathname.match(/^\/api\/projects\/([^/]+)\/path-audit\/report$/)
      if (m && method === 'POST') {
        const project = await getProject(decodeURIComponent(m[1]))
        if (!project) return sendJson(res, 404, { error: 'project not found' })
        const capErr = requireRootCapability(rootForProject(project), 'writeMeta')
        if (capErr) return sendJson(res, capErr.status, { error: capErr.error })
        const result = await scanProjectPathReferences(project)
        const saved = await savePathAuditReport(project, result)
        return sendJson(res, 200, { ok: true, ...saved, result })
      }

      m = pathname.match(/^\/api\/projects\/([^/]+)\/app-manifest$/)
      if (m && method === 'GET') {
        const project = await getProject(decodeURIComponent(m[1]))
        if (!project) return sendJson(res, 404, { error: 'project not found' })
        const manifest = readAppManifest(project)
