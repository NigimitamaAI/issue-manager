...
import { buildEnterpriseHelpers, checkExtensionCompatibility, isExtensionEnabled, CURRENT_STANDARD_API_VERSION } from './extension-api.mjs'
...
export function makeHandler(ctx) {
  ...
  function enterpriseFeatureJson(id, label) {
    return {
      id,
      label,
      modulePath: `/enterprise/${id}/${id}.js`,
      cssPath: `/enterprise/${id}/${id}.css`,
    }
  }

  function loadExtensionJson(extensionId) {
    const jsonPath = path.join(ENTERPRISE_DIR, extensionId, 'extension.json')
    if (!fs.existsSync(jsonPath)) return null
    try {
      return JSON.parse(fs.readFileSync(jsonPath, 'utf8').replace(/^\uFEFF/, ''))
    } catch (e) {
      logger.logErr(`[enterprise] extension.json 読み込みエラー (${extensionId}): ${e.message}`)
      return null
    }
  }

  function availableEnterpriseFeatures() {
    // config.json を同期読み込みして ON/OFF 判定に使う
    let config = {}
    try {
    ...
    const candidates = [
      ...
      { id: 'preview-lane',    label: 'Preview Lane',    jsFile: 'preview-lane.js' },
    ]

    const features = []
    for (const { id, label, jsFile } of candidates) {
      const dir = path.join(ENTERPRISE_DIR, id)
      if (!fs.existsSync(path.join(dir, 'routes.mjs'))) continue
      if (!fs.existsSync(path.join(dir, 'assets', jsFile))) continue

      const extJson = loadExtensionJson(id)

      // ON/OFF 判定（論点3: 案C）
      if (!isExtensionEnabled(id, extJson, config)) {
        logger.log(`[enterprise] 拡張 "${id}" は無効（enabledByDefault=false または enabledExtensions 未登録）`)
        continue
      }
      ...
      features.push(enterpriseFeatureJson(id, label))
    }
    return features
  }

  async function handleEnterpriseStatic(pathname, res) {
    const m = pathname.match(/^\/enterprise\/([A-Za-z0-9_-]+)\/([A-Za-z0-9_.-]+)$/)
    if (!m) return false
    const feature = m[1]
    const file = m[2]
    const full = path.resolve(ENTERPRISE_DIR, feature, 'assets', file)
    const assetsDir = path.resolve(ENTERPRISE_DIR, feature, 'assets')
  ...
  async function handleEnterpriseApi(req, res, pathname, method, projectId) {
    ...
    if (pathname.includes('/review/') && features.some(f => f.id === 'review-workflow')) {
      const mod = await import('../enterprise/review-workflow/routes.mjs')
    ...
    if (pathname.includes('/paper/') && features.some(f => f.id === 'paper-platform')) {
      const mod = await import('../enterprise/paper-platform/routes.mjs')
      return mod.handlePaperPlatformRoute({ req, res, pathname, method, projectId, helpers, parsedBody })
    }
    if (pathname.includes('/preview-lane/') && features.some(f => f.id === 'preview-lane')) {
      const mod = await import('../enterprise/preview-lane/routes.mjs')
      return mod.handlePreviewLaneRoute({ req, res, pathname, method, projectId, helpers, parsedBody })
    }
    return false
  }
  ...
  return async function handle(req, res) {
    ...
    try {
      ...
      m = pathname.match(/^\/api\/projects\/([^/]+)\/(paper|review|preview-lane)\/[A-Za-z0-9_-]+$/)
      if (m) {
