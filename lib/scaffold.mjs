...
import { makeProjectSharedDockerPolicy } from './shared-config.mjs'
...
Docker 共有環境を使う場合も、共有フォルダの実体パスはここに書きません。プロジェクト側は共有環境IDなどの論理参照だけを持ち、実体パスは issue_manager の設定から解決します。
...
export async function scaffoldProject(projectDir, projectName, options = {}) {
  ...
  if (!fs.existsSync(projectFilePath)) {
    const projectInfo = {
      ...
      ticketsDir: 'tickets',
      extras: {
        docker: options.dockerPolicy || makeProjectSharedDockerPolicy(),
      },
    }
...
// 起動時のディレクトリ初期化
// ────────────────────────────────────────────────
// public/ ディレクトリと埋め込みファイル群を準備。
// public/ は生成物なので、core/public-assets 側と差分があれば上書きする。
// publicFiles: { 'index.html': '...', 'favicon.png': Buffer } 形式。
export async function ensurePublic(publicDir, publicFiles, logger) {
  ...
  for (const [name, content] of Object.entries(publicFiles)) {
    ...
    await fsp.writeFile(target, content, isBinary ? undefined : 'utf8')
