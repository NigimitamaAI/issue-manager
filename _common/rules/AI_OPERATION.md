...
| 作業対象 | 必ず確認する実装 |
|---|---|
| `_docker/`、Docker確認環境、環境カタログ | `open\preview-lane\routes.mjs`、`open\preview-lane\assets\preview-lane.js`、`_share\docker\catalog.json` |
| Docker共有設定、Traefik | `lib\shared-config.mjs`、`open\preview-lane\routes.mjs`、`_share\traefik\docker-compose.yml` |
...
`open\preview-lane\routes.mjs` では、少なくとも次の関数の実挙動を確認する。
