...
$bundledFileTool = Join-Path $PSScriptRoot "_tools\Filetool.bat"
$defaultBomFix  = if ($existing -and $existing.bomFixerPath) {
    [string]$existing.bomFixerPath
} elseif (Test-Path -LiteralPath $bundledFileTool) {
    Convert-ToJsonPath $bundledFileTool
} else {
    ""
}
...
Write-Title "4/5: FileTool / BOM Fixer Path (optional)"
Write-Info "Path to Filetool.bat. It can strip UTF-8 BOM from .bat files and run file-operation instructions."
Write-Info "The bundled default is usually OK. Example:"
Write-Info "  G:/claudedir/issue_manager/_tools/Filetool.bat"
...
if (Confirm-YesNo "Change it?" "N") {
    ...
    $bomPath = Select-File -Title "Select Filetool.bat / BOM fixer (Cancel to clear)" -Filter "Batch file (*.bat)|*.bat|All files (*.*)|*.*" -InitialDir $initialDir
    if ($bomPath) {
...
$json = @"
{
  ...
  "_comment_bomFixerPath": "Path to Filetool.bat or another tool that strips UTF-8 BOM from .bat files. Leave blank if not used.",
