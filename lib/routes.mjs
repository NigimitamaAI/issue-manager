...
export function makeHandler(ctx) {
  ...
  async function saveRootsToConfig() {
    ...
    raw.roots = (args.roots || []).map(publicRootJson)
  ...
  async function getProject(projectId) {
    return P.getProjectById(projectId, args.roots, logger)
  }
  ...
  return async function handle(req, res) {
    ...
    try {
      ...
      if (pathname === '/api/fs/children' && method === 'GET') {
        ...
        const managed = await P.detectProjects(args.roots, logger)
      ...
      let m = pathname.match(/^\/api\/projects\/([^/]+)\/tickets$/)
      if (m && method === 'GET') {
      ...
      if (m && method === 'GET') {
        ...
        const targetLanes = ['inbox', 'todo', 'doing', 'blocked', 'done']
