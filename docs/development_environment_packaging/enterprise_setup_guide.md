# プレビューレーン（確認・テスト環境管理） 導入手順・システム要件

本ドキュメントは、`issue_manager` のEnterprise（有料）拡張機能である「プレビューレーン（preview-lane）」を導入する際のシステム要件、およびDocker等のインフラセットアップ手順を解説します。
...
### ステップ1: 拡張機能ファイルの配置
本拡張パッケージ（`preview-lane`）を、`issue_manager` ディレクトリ内の `enterprise` フォルダ配下にコピーします。
...
プレビューレーンからWindowsのエクスプローラーを開くには、`issue_manager` のプロジェクトルート設定において `openExternal` 権限が必要です。
...
3. 画面上部に **「🚀 プレビューレーン」** ボタンが表示されていることを確認します。
