...
- [x] 拡張機能 `preview-lane` の骨組み実装
  - [x] `enterprise/preview-lane/extension.json` の作成
  - [x] `enterprise/preview-lane/routes.mjs` のスケルトン作成
- [x] `issue_manager` 本体へのルーティング接続
  - [x] `lib/routes.mjs` を修正し、`preview-lane` 拡張を認識させてルーティングする
- [x] バックエンド（`enterprise/preview-lane/routes.mjs`）の実装
...
- [x] フロントエンドUIの実装
  ...
  - [x] `public/app.js` と `enterprise/preview-lane/assets/preview-lane.js` / `preview-lane.css` の実装（マウント機構を利用して拡張機能単体で実装完了）
