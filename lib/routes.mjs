...
export function makeHandler(ctx) {
  ...
  function enterpriseFeatureJson(id, label, tier, urlPrefix) {
    return {
      id,
      label,
      tier,
      modulePath: `${urlPrefix}/${id}/${id}.js`,
      cssPath: `${urlPrefix}/${id}/${id}.css`,
    }
  }
  ...
  function availableEnterpriseFeatures() {
    ...
    for (const tier of EXTENSION_TIERS) {
      for (const { id, label, jsFile } of tier.candidates) {
        ...
        features.push(enterpriseFeatureJson(id, label, tier.tier, tier.urlPrefix))
      }
    }
    return features
  }
  ...
  function performLicenseAudit() {
    ...
    try {
      ...
      logger.logErr(`[license] config.json 読み込み失敗: ${e.message}`)
    }
  ...
  async function handleEnterpriseApi(req, res, pathname, method, projectId) {
    ...
    if (pathname.includes('/preview-lane/') && features.some(f => f.id === 'preview-lane')) {
      const tier = findExtensionTier('preview-lane')
      const mod = await import(pathToFileURL(path.join(tier.dir, 'preview-lane', 'routes.mjs')).href)
